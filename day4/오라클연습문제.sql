...
SELECT C.NAME,ROUND(AVG(SALEPRICE),0) AS 평균주문금액
FROM CUSTOMER C
...
SELECT ORDERID,ORDERDATE,BOOKID
FROM BOOK B
INNER JOIN ORDERS O ON B.BOOID=O.BOOKID
...
SELECT NAME,PHONE,NVL(PHONE,'연락처없음')AS 연락처
FROM CUSTOMER;
...
SELECT ENAME,DNAME, SAL,ROUND (AVG_SAL,0) AS 부서평균급여
FROM EMP E
INNER JOIN(
...
SELECT U.USERNAME,NVL(TO_CHAR(ROUND(AVG(CNT),1)),'게시글없음')AS 조회수평균
FROM TBL_USER U
